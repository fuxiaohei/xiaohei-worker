 ...
 public:
  ...
  HeapObjectT *alloc() {
    ...
    if (m_begin == nullptr) {
      ...
      m_current = m_begin;
      hlogd("heap:%p, allocate %d bytes, ptr %p", this, m_alloc_size, m_begin->data);
    ...
    m_current->next = next;
    m_current = next;
    hlogd("heap:%p, allocate %d bytes, ptr %p", this, m_alloc_size, next->data);
