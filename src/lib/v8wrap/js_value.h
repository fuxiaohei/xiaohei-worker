...
template <class T> T *get_ptr(v8::Isolate *isolate, uint32_t index) {
...
template <class T> T *get_ptr(v8::Local<v8::Context> context, int index) {
...
template <class T> static T *get_ptr(v8::Local<v8::External> extValue) {
    auto v = extValue->Value();
    return static_cast<T *>(v);
}

template <class T> static T *get_ptr(v8::Local<v8::Object> handle) {
    assert(!handle.IsEmpty());
    assert(handle->InternalFieldCount() > 0);
